...
"""
    ...
    def _push_event(self, event: EventLock, github_agent: GitHubAgent) -> bool:
        ...
        try:
            ...
            # Use direct GitHub REST API to create PR
            import requests
            ...
            # Handle PR creation response
            if response.status_code == 422:
                # PR likely already exists - check error message
                error_data = response.json()
                error_msg = error_data.get('errors', [{}])[0].get('message', '') if 'errors' in error_data else ''
                
                if 'pull request already exists' in error_msg.lower() or 'already exists' in error_msg.lower():
                    print(f"  ℹ️  Pull request already exists for branch {branch_name}")
                    # Try to find existing PR
                    search_url = f"https://api.github.com/repos/{self.owner}/{self.repo}/pulls?head={self.owner}:{branch_name}&state=open"
                    ...
                    if search_response.status_code == 200:
                        ...
                        if prs:
                            ...
                            print(f"  ✓ Using existing PR: {pr_url}")
                            self.config_manager.update_event_progress(
                                event.id,
                                pushed=True,
                                pr_created=True,
                            ...
                            return True
                    print(f"  ⚠️  Branch pushed but couldn't locate existing PR")
                    self.config_manager.update_event_progress(event.id, pushed=True)
                    return True  # Still consider this a success
                else:
                    # Other 422 error
                    print(f"  ⚠️  PR creation failed: {error_msg}")
                    self.config_manager.update_event_progress(event.id, pushed=True)
                    return True  # Branch was pushed successfully
            
            response.raise_for_status()
            pr_data = response.json()
            
            pr_number = pr_data.get("number")
            pr_url = pr_data.get("html_url", f"https://github.com/{self.owner}/{self.repo}/pull/{pr_number}")
            ...
            # HTTP errors during push/PR creation - log but don't fail the whole process
            print(f"\n  ⚠️  Push/PR error: {e}")
            # Mark as pushed if branch push succeeded
            self.config_manager.update_event_progress(event.id, pushed=True)
            return True  # Continue with other events
        except Exception as e:
...
Always respond with valid JSON including your analysis and changes."""},
                    ...
                    # Note: temperature removed - some models don't support custom values with response_format
                )
