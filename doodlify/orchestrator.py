...
class Orchestrator:
    ...
    def _report_suggestions(self, suggestions: List[dict]) -> None:
        ...
        for s in suggestions:
            ...
            user_input = f"""
            Search for existing GitHub issues with title "{title}" in repository {self.owner}/{self.repo} that are issues.
...
"""
    ...
    def _push_event(self, event: EventLock, github_agent: GitHubAgent) -> bool:
        ...
        try:
            ...
            user_input = f"""
            Search for existing GitHub issues with title "{pr_title}" in repository {self.owner}/{self.repo} that are issues.
            If no duplicate exists, create a new pull request with:
            - Title: {pr_title}
            - Body: {pr_body}
